...
def check(dest_file):
    ...
    res = None
    try:
        cmd = ['genmc', '--wb', '--check-consistency-type=full', '--check-consistency-point=step', '--stop-on-race','--nthreads=8','--timeout=1500', dest_file]
    ...
    if len(races) == 0:
        ...
        if out.find('TimeOut') >=0:
            print("Till The TimeOut of 1500mins")
...
def main(filename,nrepairs):
    ...
    res = None
    try:
        cmd = ['genmc', '--wb', '--check-consistency-type=full', '--check-consistency-point=step', '--stop-on-race','--nthreads=11', '--disable-barrier-divergence','--timeout=1500',dest_file]
        out = subprocess.check_output(cmd,stderr = subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        if e.returncode != 42:
            res['failure'] = 'genmc'
