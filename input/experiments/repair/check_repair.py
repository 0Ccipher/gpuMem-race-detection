...
def check(dest_file):
    ...
    res = None
    try:
        cmd = ['genmc', '--wb', '--check-consistency-type=full', '--check-consistency-point=step', '--stop-on-race','--nthreads=11','--timeout=10', dest_file]
...
def main(filename,nrepairs):
    ...
    res = None
    try:
        cmd = ['genmc', '--wb', '--check-consistency-type=full', '--check-consistency-point=step', '--stop-on-race','--nthreads=11', '--disable-barrier-divergence','--timeout=10',dest_file]
        out = subprocess.check_output(cmd,stderr = subprocess.STDOUT)
    except subprocess.CalledProcessError as e:
        if e.returncode != 42:
            res['failure'] = 'genmc'
